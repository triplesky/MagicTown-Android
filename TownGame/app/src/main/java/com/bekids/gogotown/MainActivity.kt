...
import com.ihuman.sdk.lib.account.webkit.callback.IHAccountCallback
...
import com.ihuman.sdk.lib.webkit.jsbridge.callback.IHJsBridgeCallback
import com.ihuman.sdk.lib.webkit.jsbridge.callback.IHWebKitCallbackHandler
...
class MainActivity : AppCompatActivity() , IUnityPlayerLifecycleEvents, IHumanDataMergeCallback,
    IHAccountCallback, IHJsBridgeCallback {
...
//            build()
//        }
        IHAccountComponent.getInstance().setWebAccountListener(this)
        IHWebKitComponent.getInstance().setJsBridgeCallback(this);
...
//        Log.i("lldebug delay","before")
    ...
    override fun wantsToLogin() {
        mViewModel.login(this@MainActivity)
    }

    override fun wantsToLogout() {
        mViewModel.logout()
    }

    override fun handleUrlCallback(url: String?, callbackHandler: IHWebKitCallbackHandler?) {
        IHEventBus.instance?.post(
            BusMsg(
                BusMsg.HANDLE_URL_MSG,
                url
            )
        )
        ...
        if (action.equals("payments")) {
            ...
            val id = uri.getQueryParameter("productId")
            if (result?.toInt() == 1) {
                if (!id.isNullOrEmpty()) {
                    mViewModel.getUserInfo()
                    var data = Payment(0, V(0, ProductPay(id), 0,""))
                    IHEventBus.instance?.post(BusMsg(BusMsg.PAY_PURCHASE_ALL_PACK_WITH_PRODUCT_ID_COMPLETE, data))
                }

            }else{
                if (!id.isNullOrEmpty()) {
                    var data = Payment(-1, V(-1, ProductPay(id), 0,""))
                    IHEventBus.instance?.post(BusMsg(BusMsg.PAY_PURCHASE_ALL_PACK_WITH_PRODUCT_ID_COMPLETE, data))
                }

            }
        } else if (action.equals("open_app")) {
            val pack_name = uri.getQueryParameter("pack_name")
            if (!pack_name.isNullOrEmpty()) {
                if (IHMarketUtils.canOpenApp(pack_name)) {
                    IHMarketUtils.openApp(pack_name)
                } else {
                    IHMarketUtils.gotoMarket(pack_name)
                }
            }
        }
        callbackHandler?.invokeSuccessCallback()
    }

    override fun webViewOnClose() {
