...
import com.ihuman.sdk.lib.thirdapi.pay.model.IHProductType
import com.ihuman.sdk.lib.thirdapi.pay.model.ProductType
...
//                        right.put("plus_vip_expire_time", null)
            ...
            MessageConstants.MESSAGE_PAY_PRODUCT_INFO_FOR_USER_RIGHTS_TYPE -> {
                ...
                if (product != null) {
                    ...
                    result = generateObjJson(JSONObject())
                }
            }

            MessageConstants.MESSAGE_USER_STATUS -> {
                Log.i("lldebug user_status", "")
    ...
    fun onEvent(event: BusMsg<*>) {
        when (event.code) {
            ...
            BusMsg.HANDLE_URL_MSG -> {
                var jsonObject = JSONObject()
                jsonObject.put("url", event.data as String)
                callUnity(
                    MessageConstants.MESSAGE_HANDLE_URL,
                    jsonObject.toString(),
                    "",
                    MessageConstants.MESSAGE_HANDLE_URL
                )
