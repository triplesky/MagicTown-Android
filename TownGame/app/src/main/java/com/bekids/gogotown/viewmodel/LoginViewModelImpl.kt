...
import com.ihuman.sdk.lib.thirdapi.pay.IHumanPayCallback
import com.ihuman.sdk.lib.thirdapi.pay.IHumanQuerySkuCallback
import com.ihuman.sdk.lib.thirdapi.pay.IHumanResumeAllPurchaseCallback
import com.ihuman.sdk.lib.thirdapi.pay.IHumanResumePurchaseCallback
import com.ihuman.sdk.lib.thirdapi.pay.model.*
            ...
            withContext(Dispatchers.Main){
                if (result is IHumanResult.Success) {
                    if (result.isOk()) {
                        ...
                        MMKvUtils.put("app_config_list", result.result!!.app_config_list)
                        var skuLists_temp = mutableListOf<SkuDetails>()
                        var l = mutableListOf<PurchaseBean>()
                        //IHEventBus.instance?.post(BusMsg(BusMsg.PAY_FETCH_SERVER_PRODUCT_LIST_COMPLETE, null))

                        for ((i,product) in product_list.withIndex()){
                            var list = mutableListOf<String>()
                            list.add(product.product_id)
                            AccountHelp.getInstance().querySkuDetails(MainApplication.mainActivity, list,getProductEnum(product.product_type),
                                object : IHumanQuerySkuCallback {
                                    override fun onResult(skuLists: MutableList<SkuDetails>?) {
                                            if (!skuLists.isNullOrEmpty()) {
                                                skuLists_temp.addAll(skuLists)
                                                Log.i("lldebug size", i.toString() +", "+product_list.size)

                                            }
                                            if (i == product_list.size -1) {
                                                for (skuitem in skuLists_temp) {
                                                    for (item in product_list) {
                                                        if (item.product_id == skuitem.sku) {
                                                            var data =
                                                                PurchaseBean(item.discount.toDouble(),
                                                                    item.product_id,
                                                                    item.product_type,
                                                                    item.rights_type,
                                                                    Ih(skuitem.price,
                                                                        skuitem.sku,
                                                                        (skuitem.priceAmountMicros / 10000).toInt()
                                                                    ))
                                                            l.add(data)
                                                        }
                                                    }
                                        ...
                                        if (l.isEmpty()) {
                                            IHEventBus.instance?.post(BusMsg(BusMsg.PAY_FETCH_ALL_PACK_PRODUCT_LIST_COMPLETE,null))
                                        }else{
                                            var prosku =  ProductSkuBean(result.result!!, skuLists_temp)
                                            ProductListHolder.saveProduct(Gson().toJson(prosku), prosku)
                                            IHEventBus.instance?.post(BusMsg(BusMsg.PAY_FETCH_ALL_PACK_PRODUCT_LIST_COMPLETE,l))
                                        }
//                                            else {
//                                                IHEventBus.instance?.post(BusMsg(BusMsg.PAY_FETCH_ALL_PACK_PRODUCT_LIST_COMPLETE,null))
//                                                return
//                                            }
    ...
    fun getProductEnum(type:Int):ProductType{
